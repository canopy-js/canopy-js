...
function build(options) {
  ...
  if (!manualHtml) {
    ...
    let html = dedent`
      ...
      <head>
      <meta charset="utf-8">` +
      dedent`${customCss ? `<link rel="stylesheet" href="${projectPathPrefix ? '/' + projectPathPrefix :''}/_assets/custom.css" media="print">\n` : ''}` + // async loading
      ...
      dedent`<link rel="preload" href="${projectPathPrefix ? '/' + projectPathPrefix :''}/_data/${defaultTopic.fileName}.json" as="fetch" crossorigin="anonymous" fetchpriority="low">` + '\n' +
