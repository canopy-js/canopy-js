...
const bulk = async function(selectedFileList, options) {
  ...
  if (options.sync) {
    ...
    watch(Object.assign({ ...options, ...{ suppressInitialBuild: determineWhetherToBuild() }}));
...
function determineWhetherToBuild() {
  var topicsStats = fs.statSync("./topics");
  var topicsMtime = topicsStats.mtime;

  if (!fs.existsSync('./build')) {
    return true;
  } else {
    var buildStats = fs.statSync("./build");
    var buildMtime = buildStats.mtime;
  }

  return topicsMtime < buildMtime;
}
