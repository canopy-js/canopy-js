...
class ParserContext {
  ...
  formatErrorWithContext(message, filePath, line, col, contextRadius = 1) {
    if (!filePath || !line || line < 1) return message;

    let contents = this.explFileObjectsByPath?.[filePath]?.contents || null;
    if (!contents) return message;

    let lines = contents.split('\n');
    let start = Math.max(0, line - 1 - contextRadius);
    let end = Math.min(lines.length, line - 1 + contextRadius);
    let width = String(end + 1).length;
    let caret = col && col > 0 ? col : 1;
