      ...
      scanFileSystem({
        ...
        onNoChange: (filePath) => {
          if (originalFilePaths.includes(filePath)) {
            ...
            if (daysSinceLastReview <= 2 * interval) {
              console.error(1, daysSinceLastReview, interval, 2 * interval)
              review.iterations++;
            } else {
              let overdue = daysSinceLastReview - 2 * interval;
              let penalty = 0;
              let bandSize = interval;

              while (overdue > 0 && penalty < n) {
                penalty++;
                overdue -= bandSize;
                bandSize = Math.floor(bandSize / 2);
              }

              let newIterations = Math.max(0, n - penalty);

              log(chalk.gray(
                `Review late by ${daysSinceLastReview} days (interval ${interval}). ` +
...
function scanFileSystem({
  ...
  onAddition, onDeletion, onChange, onNoChange, path, now
}) {
