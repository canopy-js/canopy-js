...
function parseLine(line, tokens, parserState) {
  if (line.match(/^```\s*/) || (tokens[tokens.length - 1]?.type === 'code_block' && tokens[tokens.length - 1]?.open)) {
    ...
    handleOutline(line, tokens, parserState);
  } else if (line.match(/^\|([^|\n]*\|)+/) || (line.match(/^\s*[=#-]+\s*$/) && tokens[tokens.length - 1]?.type === 'table')) {
    handleTable(line, tokens, parserState)
  } else if (line.match(/^<\w+>.*<\/\w+>/)) {
    handleHtml(line);
  } else if (line.match(/^\s*\[\^[^\]]+]\:/)) {
    handleFootnote(line, tokens, parserState);
  } else {
    handleText(line, tokens, parserState);
  }
...
function handleOutline(line, tokens, parserState) {
  let outlineToken;
...
function handleTable(line, tokens, parserState) {
  if (line.match(/^\s*[=#-|]+\s*$/)) {
  ...
  let tokensByCell = cellStrings.map(
    (cellString) => parseText(cellString, parserState)
  );
...
function handleFootnote(line, tokens, parserState) {
  let match = line.match(/^\[\^([^\]]+)]\:(.*$)/);
  let superscript = match[1];
  let text = match[2];
  let footnoteTokens = parseText(text, parserState);
...
function handleText(line, tokens, parserState) {
  parseText(line, parserState).forEach(token => {
    tokens.push(token);
  });
}
