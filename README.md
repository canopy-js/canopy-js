...
When an expert produces an explanation, they are taking their interconnected knowledge and producing a partial linearization of it for some specific audience or purpose, creating a new composition from a set of reusable parts. This linearization can occur up-front, when the author produces a book or lecture. Or, the expert can do it on the fly, producing small linearizations in response to particular questions. This type of custom explanation can include more clarification for a beginner, more detail for someone advanced, or connections between the material and the particular interests of the listener. Customization, however, requires continued access to the explainer, which isn't always a possibility. So, instead of having the expert produce a single text, the author might instead record their graph itself, allowing the user to produce linearizations on their own, customized to their needs, without requiring continued access to the explainer.

#### 2. Explanations can be very large

There are practical limitations on the length of a book, or the size of a lecture, but experts have the ability produce explanations much larger than this. A professor can explain the material of an entire course. A domain expert can explain the content of an entire discipline. Producing large explanations like these as one unit might confer certain benefits. A singly-authored work would have a common vocabulary and consistent approach that might make it easier to assimilate the various pieces. Large works might  have more room to capture various subdomains and interconnections that get lost between more popular subjects. Such a work might also capture the subtle decisions an expert makes in choosing how to structure their understanding of the corpus as a whole. Yet dealing with large explanations presents logistical difficulty. At the moment, they are hard to produce, and they are also hard to consume. It would take too long to find things in them manually, and beyond a certain scale, even the tables of contents would become too large to navigate. Yet perhaps we can borrow a helpful finding from computer science, which is that trees scale much more gracefully than lists. By composing large explanations out of a graph of trees, even a set of a million paragraphs could be navigated in just a handful of steps. And while in a text space for one thing trades off with another, in a graph there is unlimited space. So, by making it tractable to consume such large explanations, we might find that new kinds of "writing in the large" become possible.

#### 3. Mastery requires dense graphs, which are hard to serialize

Two people can know the same facts, but may have varying degrees of comfort and flexibility interacting with them. What distinguishes the expert is that they not only know many things, but they have many connections between them – one fact proves another, one event preceded another – and not only can they derive these relations, but very often they have them already committed to memory, and in multiple places, to be accessed instantly without having to leave the subject at hand. What results from this is a dense graph where every node has many edges. Yet dense graphs are very difficult to serialize, especially in narrative prose which favors forward-driving action to combinatorial interrelation. In times of expensive paper, authors have often had to write only the essential facts, requiring the reader to do the "unpacking", finding the derivative points and enumerating the interconnections, yet many lack the skill to do so, memorizing information in the arbitrary form it was presented. Thus, there is very likely a chronic under-supply of this redundant interconnective data in existing explanatory mediums. A solution would be to have the expert capture and produce their underlying graph, so that consumers have the ability to explore the redundant connections that form mastery at their leisure, hopefully increasing the fidelity and ease with which complicated ideas can be transferred.
