...
When an expert produces an explanation, they are taking their interconnected knowledge and producing a partial linearization of it for some specific audience or purpose, creating a new composition from a set of reusable parts. This linearization can occur up-front, when the author produces a book or lecture. Or, the expert can do it on the fly, producing small linearizations in response to particular questions. This type of customized explanation can include more clarification for a beginner, more detail for someone advanced, or connections between the material and the background of the listener. Customization, however, requires the listener to have continued access to the explainer, which isn't always possible. So, rather than have the expert produce a single text, the author might instead record the graph itself, allowing the user to produce linearizations on their own, customized to their interests, without having to have continued access to the explainer.
